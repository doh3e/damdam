...
pipeline {
  ...
  environment {
    ...
    COMPOSE_PROJECT_NAME = "k12s202-develop"
    // Use the host’s Docker daemon socket
    DOCKER_HOST         = 'unix:///var/run/docker.sock'
    // Store Docker CLI config in the workspace to avoid permission issues
  ...
  stages {
    stage('Checkout SCM') {
      steps {
        // Clone the repository so that docker-compose.yml is in ${WORKSPACE}
        checkout scm
      }
    }
    ...
    stage('Build & Deploy') {
      steps {
        ...
        // Bring up all services (rebuilding if needed)
        sh 'docker-compose up --build -d'
      }
    }
